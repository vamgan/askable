...
| `useAskableDialogSource` | Open modals, drawers, popovers — title, actions, stack depth |
...
- **Dialog awareness** — `useAskableDialogSource()` reports which modal, drawer, or popover is open, its title, actions, and stack depth; `autoDetect` reads native `<dialog>`, ARIA dialogs, and popovers straight from the DOM
